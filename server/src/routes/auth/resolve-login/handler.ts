...
export const handleResolveLogin = async (req: Request, res: Response) => {
  const parsed = loginSchema.safeParse({ email: req.body.email ?? req.body.username, password: req.body.password });
  if (!parsed.success) return res.status(400).json({ error: "A valid email and password are required", details: parsed.error.flatten().fieldErrors });
  ...
  try {
    ...
    if (platformUser && await bcrypt.compare(password, platformUser.password_hash)) {
      ...
      const accessToken = jwt.sign({ userId: platformUser.id, email, isPlatformUser: true }, env.JWT_SECRET, { expiresIn: "24h" });
