...
export const handleListLostFound = async (req: AuthorizedRequest, res: Response) => {
  ...
  const posts = await prisma.lostFoundPost.findMany({ where: { org_id: orgId }, include: { author: { select: { full_name: true } } }, orderBy: { created_at: "desc" }, take: limit, skip });
  ...
  return res.json({ posts: posts.map((post) => ({ id: post.id, body: post.caption, imageUrls: post.image_urls, publisherName: post.author.full_name, createdAt: post.created_at, reactionCount: interactions.filter((item) => item.post_id === post.id && item.kind === "reaction").length, commentCount: interactions.filter((item) => item.post_id === post.id && item.kind === "comment").length, comments: interactions.filter((item) => item.post_id === post.id && item.kind === "comment").map((item) => ({ id: item.id, body: item.body, authorName: item.user.full_name })) })), pagination: paginationMeta(page, limit, posts.length) });
};
